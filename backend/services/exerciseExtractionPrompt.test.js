...
const blank = { chapterName: 'Fractions', topics: ['Addition'], label: 'Fill in the blanks — missing numerator', questionType: 'short-answer', instructions: 'Fill the missing numerator', example: '$1/5 + 2/5 = \\frac{\\square}{5}$', sourcePages: [4] };
const originalFetch = global.fetch;
beforeEach(() => { global.fetch = jest.fn(); });
afterEach(() => { global.fetch = originalFetch; });
const respond = review => global.fetch.mockResolvedValue({ ok: true, json: async () => ({ output_text: JSON.stringify(review) }) });
...
test('dedicated prompt reviews the original PDF, and records safe auditable evidence', async () => {
  ...
  const result = await extractExercisePatterns(file, course, 2, { onExchange });
