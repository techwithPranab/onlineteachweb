...
describe('course scan material mapping', () => {
  ...
  test('matching metadata is retained while content comes from OCR', async () => {
    ...
    expect(result.materials[0]).toEqual({ title: 'AI title', description: 'Summary', chapterName: 'Chapter', content: 'Full source content', exercisePatterns: [] });
  });
  test('retains exercise formats on their original PDF when material metadata count differs', async () => {
    const patterns = [
      { sourceFileIndex: 0, chapterName: 'Fractions', topics: ['Addition'], label: 'Fill in the blanks', questionType: 'short-answer', instructions: 'Complete the blanks', example: '1/5 + 2/5 = ___' },
      { sourceFileIndex: 1, chapterName: 'Plants', topics: [], label: 'True or false', questionType: 'true-false', instructions: 'State true or false', example: 'Plants need water.' }
    ];
    global.fetch.mockResolvedValueOnce(respond('Fraction exercises')).mockResolvedValueOnce(respond('Plant exercises'))
      .mockResolvedValueOnce(respond(JSON.stringify({ course: { title: 'Course', exercisePatterns: patterns }, materials: [] })));
