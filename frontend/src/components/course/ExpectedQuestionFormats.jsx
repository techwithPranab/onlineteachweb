...
import { QUESTION_TYPE_LABELS, mergeExpectedFormats, getResponseQuestions, safePdfUrl } from '@/utils/courseQuestionFormats.mjs'

export default function ExpectedQuestionFormats({ course, materials = [] }) {
  ...
  const formats = mergeExpectedFormats(structure.data?.exercisePatterns || course.exercisePatterns || [], generations)
  ...
  return <section className="bg-white rounded-lg shadow p-6" aria-labelledby="expected-question-formats">
    <h2 id="expected-question-formats" className="text-xl font-semibold">Expected question formats</h2>
    ...
    {(structure.isLoading || history.isLoading) && <p role="status" className="text-sm text-gray-500 mt-4">Loading exercise formats and prompt results…</p>}
