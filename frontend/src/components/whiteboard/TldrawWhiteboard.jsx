...
export default function TldrawWhiteboard({ socket, sessionId, userRole, className = '' }) {
  ...
  const [mathFormulas, setMathFormulas] = useState([]) // Store formulas with positions
  ...
  useEffect(() => {
    ...
    // Math formula listeners
    socket.on('math:add', (data) => {
      if (data.formula) {
        setMathFormulas(prev => [...prev, data.formula])
      }
    })
    
    socket.on('math:remove', (data) => {
      if (data.formulaId) {
        setMathFormulas(prev => prev.filter(f => f.id !== data.formulaId))
      }
    })
  ...
  const addMathFormula = (latex = null) => {
    ...
    // Add formula to state with a random position offset to avoid stacking
    const offsetX = Math.random() * 200 - 100
    const offsetY = Math.random() * 100 - 50
    
    const newFormula = {
      id: `formula_${Date.now()}_${Math.random().toString(36).substring(2)}`,
      latex: formula,
      x: 50 + offsetX, // percentage from left
      y: 50 + offsetY, // percentage from top
      createdAt: Date.now()
    }
    
    setMathFormulas(prev => [...prev, newFormula])
    
    // Broadcast to other users
    if (socket && sessionId) {
      socket.emit('math:add', {
        sessionId,
        formula: newFormula
      })
    }

    console.log('Math formula added:', formula)

    // Clear input if it was from manual entry
    if (!latex) {
      setMathInput('')
  ...
  // Remove a math formula
  const removeMathFormula = (id) => {
    setMathFormulas(prev => prev.filter(f => f.id !== id))
    
    // Broadcast removal
    if (socket && sessionId) {
      socket.emit('math:remove', {
        sessionId,
        formulaId: id
      })
    }
  ...
  return (
    <div className={`relative bg-white rounded-lg shadow-lg ${className}`}>
      ...
      <div className="h-96 w-full rounded-lg overflow-hidden border border-gray-300 relative">
        ...
        {/* Math Formula Overlays - Rendered on top of whiteboard */}
        {mathFormulas.map((formula) => (
          <div
            key={formula.id}
            className="absolute bg-white border-2 border-blue-500 rounded-lg px-3 py-2 shadow-lg cursor-move group"
            style={{
              left: `${formula.x}%`,
              top: `${formula.y}%`,
          ...
          >
            <button
              onClick={() => removeMathFormula(formula.id)}
